  ...
  _p[12] = {
      value: function(require, exports, module) {
           ...
           */
              ...
              function editText() {
                  var node = minder.getSelectedNode();
                  console.log('dasdasdas'+node, node.parent)
                  if(node.parent.data.fieldType == "0"){
                    node.data.fieldType = "1"
                  } else if(node.parent.data.fieldType == "1"){
                    node.data.fieldType = "0"
                  }
  ...
  _p[15] = {
      value: function(require, exports, module) {
          function NodeRuntime(type) {
              ...
              // var buttons = [ "前移:Alt+Up:ArrangeUp","后移:Alt+Down:ArrangeDown", "删除:Delete|Backspace:RemoveNode"];
              var AppendLock = 0;
                  ...
                  main.button({
                      ...
                      action: function() {
                          if (command.indexOf("Append") === 0) {
                              AppendLock++;
                              let selNode = minder.getSelectedNode();
                              console.log('选中节点：', selNode);
                              let nodeChild = selNode.children;
                              for(let i of nodeChild){
                                  if(i.data.materialId){
                                      var messageParam = {
                                          type: 'message',
                                          messageInfo: '当前为叶子节点，不可添加子节点',
                                          messageType: 'warning'
                                      };
                              ...
                              minder.execCommand(command, "分支主题");// 执行复制
  ...
  angular.module('kityminderEditor').service('revokeDialog', ['$modal', 'minder.service', function($modal, minderService) {
      ...
      minderService.registerEvent(function() {
          ...
          minder.on('importNodeData', function() {
              ...
              let str = ''
              minder.Text2Children(minder.getSelectedNode(), str);
          ...
          minder.on('exportNodeData', function() {
              ...
              let res = Node2Text(selectedNode);
              // console.log(res);
      ...
      .directive('navigator', ['memory', 'config', function(memory, config) {
          return {
              ...
              link: function(scope) {
                  ...
                  // 表示可视区域的矩形
                  var visibleRect = paper.put(new kity.Rect(100, 100).stroke('#fff', '1%'));
