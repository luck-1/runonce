...
// import util from "@/build/util.js";
export default {
  name: "menu-manage",
  data() {
    return {
  ...
  methods: {
    ...
    getDictPermissions() {
      menuService.getDictDataByType("permission_type").then(res => {
        if (res.code === 0) {
    ...
    async getAllList() {
      this.loading = true;
      let res = await menuService.getAllPermissionList();
       this.loading = false;
      if (res.code === 0) {
          // 仅展开指定级数 默认所有展开
    ...
    search() {
      if (this.searchKey) {
        this.loading = true;
        menuService.searchPermission({ title: this.searchKey }).then(res => {
          this.loading = false;
          if (res.code === 0) {
            this.data = res.obj;
          }
    ...
    submitEdit() {
      this.$refs.menuForm.validate(valid => {
        if (valid) {
          ...
          this.menuForm.permTypes=[];
          menuService.editPermission(this.menuForm).then(res => {
    ...
    //菜单弹框提交
    // async addPermission(){
    //   let  res = await addPermission(param);
    // },
    submitAdd() {
      this.$refs.menuFormAdd.validate(valid => {
       ...
       menuService.addPermission(this.menuFormAdd).then(res => {
            this.submitLoading = false;
            if (res.code === 0) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          menuService.deletePermission(ids).then(res => {
            if (res.code === 0) {
