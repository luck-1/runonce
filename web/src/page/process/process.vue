...
<!-- :show-close="false" -->
  <el-dialog :before-close="handleClose" :visible="processVisible" append-to-body  custom-class="wangxiaoniqidemingzi"
    ...
    <div style="height: calc(100vh - 10px)" >
      <iframe id="processiframes" :src='iframeUrl' frameborder="0" width="100%" height="100%"></iframe>
    </div>
  </el-dialog>
</template>
  ...
  watch: {
    processVisible: function(newVal, oldVal) {
      ...
      if(newVal) {
        ...
        this.$nextTick(()=>{
          let iframe = document.getElementById("processiframes");
  ...
  methods: {
    handleClose(done) {
      this.$confirm('关闭之前是否保存', {
        ...
        document.getElementById('processiframes').contentWindow.postMessage(sendData,'*');
      }).catch(()=>{
        this.$emit('close')
      })
    },
    /**
     * 保存后
     * this.$emit(processUpload, {json:'123',imgUrl:''})
     */
     getProcessData() {
    ...
    async addProcess(val,name,params,isClose){
      ...
      if(res.code === 0){
        ...
        this.$emit('processUpload', {json: val.xml, imgUrl: client.signatureUrl(`${res.obj}`, { expires: 80000 })})
        if (isClose) {
          this.$emit('close')
        }
      } else {
